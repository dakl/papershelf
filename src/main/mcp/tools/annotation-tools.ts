...
export function registerAnnotationTools(server: McpServer, isEnabled: (name: string) => boolean): void {
  if (isEnabled('list_annotations'))
    server.tool(
      'list_annotations',
      'List all annotations (highlights, sticky notes) for a paper. Returns annotations grouped by page.',
      {
  ...
  if (isEnabled('add_sticky_note'))
    server.tool(
      'add_sticky_note',
      'Add a sticky note annotation to a paper PDF at a specific position.',
      {
        paper_id: z.string().describe('Paper ID (UUID) or arXiv ID'),
  ...
  if (isEnabled('remove_annotation'))
    server.tool(
      'remove_annotation',
      'Remove an annotation from a paper PDF by its name identifier.',
      {
        paper_id: z.string().describe('Paper ID (UUID) or arXiv ID'),
